...
them the MEP on screen turned up for, which is the number that decides whether
they are in the network at all: `network.py` computes it as `raw_counts`, uses
it to apply the participation filter, and throws it away.
...
* **Abstentions do not count.** They are excluded from the similarity measure
  entirely (see `network.py`), so an MEP who abstained on a vote contributed
  nothing to their position and is not credited with it here either.
...
def count_mandate(mandate):
    """Distinct non-abstention votes cast, per MEP, overall and per subject.

    Returns (sessions_total, sessions_by_subject, per_mep, stats).
    """
    ...
    for session in iter_json_array(str(path)):
        ...
        for vote_type, block in (session.get("votes") or {}).items():
            ...
            if config.VOTE_WEIGHTS[vote_type] == 0:
    ...
    return sessions_total, dict(sessions_by_subject), (totals, by_subject), stats
...
def run(report, mandates=None):
    ...
    for mandate in mandates:
        sessions_total, sessions_by_subject, (totals, by_subject), stats = count_mandate(
            mandate
        )
