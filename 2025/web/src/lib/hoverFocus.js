...
import { familyOf, sameGroup } from "./families.js";
 ...
 * matrix, a delegation on a dial, the four groups in a winning coalition —
 ...
 * the whole chamber even when the network on screen is one country, so a group
 * can be hovered that has no node here. Fading all 21 Portuguese MEPs to say
 * "no Greens" is a worse answer than not fading. That silence is also why
 * `matchesSelector` compares group *identity* and not group id — a spelling
 * mismatch would look exactly like this deliberate no-op.
 *
...
function matchesSelector(node, selector) {
  ...
  // Compared by identity rather than by string: the coalition panel names
  // groups as the roll-call data spells them ("The Left", "Renew") and the
  // nodes carry the precomputed networks' spelling ("GUE/NGL", "RE"). An
  // equality test here dims nothing at all for two of term 10's eight groups,
  // and does it silently, because a focus that matches no node is dropped.
  if (selector.group && !sameGroup(node.groupId, selector.group)) return false;
